...
const initAudioPlayers = () => {
  ...
  players.forEach((player) => {
    ...
    // Throttle timeupdate to ~4fps for performance (250ms)
    // Native event fires 4-66 times per second which is excessive
    let lastTimeUpdate = 0;
    const TIME_UPDATE_THROTTLE_MS = 250;

    audio.addEventListener('timeupdate', () => {
      const now = performance.now();
      if (now - lastTimeUpdate < TIME_UPDATE_THROTTLE_MS) {
        return;
      }
      lastTimeUpdate = now;
      updateProgress();
